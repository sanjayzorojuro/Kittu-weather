...
map.on("click", function (e) {
  ...
  currentMarker = L.marker([lat, lng])
    .addTo(map)
    .bindPopup(
      `<b>Loading weather data...</b><br>Lat: ${lat.toFixed(
        2
      )}<br>Lon: ${lng.toFixed(2)}`
    )
    .openPopup();
  ...
  fetch(
    ...
    .then((data) => {
      ...
      currentMarker.setPopupContent(
        `<b>${cityName}${
          country ? ", " + country : ""
        }</b><br>Lat: ${lat.toFixed(2)}<br>Lon: ${lng.toFixed(2)}`
      );
  ...
  fetch(
    ...
    .then((data) => {
      ...
      currentMarker = L.marker([lat, lon])
        .addTo(map)
        .bindPopup(
          `<b>${name}, ${country}</b><br>Lat: ${lat.toFixed(
            2
...
/**
 * Loads the city saved in localStorage from the dashboard page.
 */
function loadCityFromDashboard() {
  const city = localStorage.getItem("selectedCity");
  if (city) {
    showLoading();

    // Fetch coordinates for the saved city
    fetch(
      `https://api.openweathermap.org/geo/1.0/direct?q=${city}&limit=1&appid=${API_KEY}`
    )
      .then((res) => res.json())
      .then((data) => {
        if (data.length === 0) {
          console.error("City from dashboard not found:", city);
          hideLoading();
          return;
        }
        const { lat, lon, name, country } = data[0];

        // Use flyTo for a smooth zooming animation
      ...
      .catch((err) => {
        console.error("Error fetching city data from dashboard:", err);
        hideLoading();
      });
  }
}
...
window.addEventListener("load", loadCityFromDashboard);
